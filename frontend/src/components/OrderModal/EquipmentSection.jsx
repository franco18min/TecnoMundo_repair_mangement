...
export function EquipmentSection({ mode, permissions, formData, handleFormChange, deviceTypes, sparePartStatus, setSparePartStatus, unlockMethod, setUnlockMethod, handlePatternChange, isPatternValue }) {
    return (
        <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    {permissions.canEditInitialDetails ? (
                        <FormField as="select" label="Tipo de Dispositivo" id="device_type_id" name="device_type_id" value={formData.device_type_id} onChange={handleFormChange} required>
                ...
                {mode === 'create' && <div className="md:col-span-2"><TextAreaField label="Repuestos Utilizados (Inicial)" name="parts_used" value={formData.parts_used} onChange={handleFormChange} /></div>}

                <div className="md:col-span-2">
                    {permissions.canEditInitialDetails ?
                        <div className="flex items-center justify-between gap-4 py-2"><label className="text-sm font-medium text-gray-700">Origen del Repuesto</label><div className="flex bg-gray-100 rounded-lg p-1 w-full max-w-xs"><button type="button" onClick={() => setSparePartStatus('local')} className={`w-1/2 p-2 rounded-md font-semibold flex items-center justify-center gap-2 transition-colors duration-200 ${sparePartStatus === 'local' ? 'bg-indigo-600 text-white shadow' : 'text-gray-600'}`}><Store size={16}/> En Stock</button><button type="button" onClick={() => setSparePartStatus('pedido')} className={`w-1/2 p-2 rounded-md font-semibold flex items-center justify-center gap-2 transition-colors duration-200 ${sparePartStatus === 'pedido' ? 'bg-indigo-600 text-white shadow' : 'text-gray-600'}`}><Package size={16}/> Para Pedir</button></div></div>
                        : null
                    }
                </div>
