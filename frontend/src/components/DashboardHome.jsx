...
export function DashboardHome({ onNewOrderClick, onViewOrderClick }) {
  // 1. Obtenemos los datos y permisos directamente de los contextos globales
  const { currentUser, orders } = useAuth();
  const { canCreateOrders } = usePermissions();

  // 2. Ya no necesitamos isLoading ni un useEffect para buscar datos.
  // La lista de órdenes se actualiza automáticamente a través del WebSocket en AuthContext.

  // 3. Calculamos las 6 órdenes más recientes a partir de la lista global
  const recentOrders = useMemo(() => {
    // Aseguramos que 'orders' exista y sea un array antes de cortarlo
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-8">
        ...
        {/* El botón de crear orden ahora está controlado por los permisos */}
        {canCreateOrders && (
            <motion.button
      ...
      {/* 4. La sección de las tarjetas ahora está envuelta en AnimatePresence */}
      <motion.div
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"
        layout // Añadimos 'layout' para que las tarjetas se reorganicen con una animación suave
      >
        <AnimatePresence>
