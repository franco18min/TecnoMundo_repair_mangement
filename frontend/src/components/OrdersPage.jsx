...
import { PlusCircle, Edit, Trash2, Wrench, CheckCircle, AlertTriangle, Clock, RotateCcw } from 'lucide-react';
...
const FilterInput = ({ label, name, value, onChange }) => (
    <div>
        ...
        <input
            type="text"
            name={name}
            value={value} // Corregido: antes era value={onChange}
            onChange={onChange} // Corregido: se añade el manejador onChange
...
const FilterSelect = ({ label, name, value, onChange, options }) => (
    <div>
        <label className="block text-xs font-medium text-gray-600 mb-1">{label}</label>
        <select name={name} value={value} onChange={onChange} className="w-full bg-gray-50 border border-gray-300 rounded-lg py-2 px-3 text-sm h-10 focus:outline-none focus:ring-2 focus:ring-indigo-500">
...
export function OrdersPage({ onNewOrderClick }) {
    ...
    const statusStyles = useMemo(() => ({
        'En Progreso': { badge: 'bg-yellow-100 text-yellow-800', icon: <Wrench size={14} className="text-yellow-600" /> },
        'Completado': { badge: 'bg-green-100 text-green-800', icon: <CheckCircle size={14} className="text-green-600" /> },
        'Pendiente': { badge: 'bg-red-100 text-red-800', icon: <AlertTriangle size={14} className="text-red-600" /> },
        'En Espera de Pieza': { badge: 'bg-blue-100 text-blue-800', icon: <Clock size={14} className="text-blue-600" /> },
    }), []);
    ...
    const uniqueStatuses = useMemo(() => ['Todos', ...new Set(orders.map(order => order.status))], [orders]);
    ...
    <>
      ...
      <motion.div className="bg-white p-4 rounded-lg shadow-md mb-6 border border-gray-200" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8 gap-4 items-end">
            ...
            <div className="xl:col-span-1"><FilterSelect name="device_type" label="Tipo Disp." value={filters.device_type} onChange={handleFilterChange} options={uniqueDeviceTypes} /></div>
            <div className="xl:col-span-1"><FilterSelect name="status" label="Estado" value={filters.status} onChange={handleFilterChange} options={uniqueStatuses} /></div>
            <div className="xl:col-span-1"><FilterSelect name="technician" label="Técnico" value={filters.technician} onChange={handleFilterChange} options={uniqueTechnicians} /></div>
        ...
        <table className="w-full min-w-[900px]">
          ...
          <tbody>
            ...
            {isLoading ? (
              ...
              const currentStatus = statusStyles[order.status] || { badge: 'bg-gray-100 text-gray-800', icon: null };
              return (
