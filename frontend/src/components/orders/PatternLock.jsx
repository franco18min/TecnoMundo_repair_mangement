...
const PatternLock = ({ onPatternComplete, displayPattern = null, readOnly = false, initialPattern = null }) => {
    ...
    const GRID_SIZE = 3;
    const DOT_RADIUS = 12; // Increased slightly for number visibility
    ...
    return (
        <div className="flex flex-col items-center">
            <svg
                ...
                width={SVG_SIZE}
                height={SVG_SIZE}
                className={`bg-gray-50 border border-gray-300 rounded-lg touch-noneSelect-none ${readOnly ? 'cursor-default' : 'cursor-pointer'}`}
            ...
            >
                ...
                {points.map(p => {
                    const index = currentPath.findIndex(cp => cp.id === p.id);
