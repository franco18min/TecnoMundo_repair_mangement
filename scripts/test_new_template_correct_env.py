#!/usr/bin/env python3
"""
Script para probar el nuevo template de email con variables de entorno correctas
"""
...
async def test_new_template_with_correct_env():
    """Prueba el nuevo template de email con diseño actualizado usando variables de entorno correctas"""
    
    print("🧪 Probando nuevo template de email con variables de entorno correctas...")
    ...
    test_data = {
        ...
        "mensaje": "Su equipo ha sido diagnosticado y está listo para reparación.",
    ...
    try:
        ...
        print("📤 Enviando email con nuevo template...")
        ...
        html_file = Path(__file__).parent / "email_preview_correct_env.html"
        ...
        resultado = email_service.send_email(
            to_email=test_data["cliente_email"],
            subject=f"Actualización de estado - Orden {test_data['numero_orden']}",
            html_content=html_content
        )
        
        if resultado:
            print("✅ Email enviado exitosamente con nuevo template!")
            print(f"📊 Resultado: {resultado}")
            print("🎨 El nuevo template incluye:")
            print("   - Diseño moderno con colores TecnoMundo (índigo)")
            print("   - Logo de la empresa integrado")
...
if __name__ == "__main__":
    asyncio.run(test_new_template_with_correct_env())